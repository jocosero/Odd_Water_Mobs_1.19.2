...
public class HorseshoeCrabEntity extends OddWaterMob implements SeafloorAnimal, IAnimatable {
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if (this.isUpsideDown()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.horseshoe_crab.fall_over", true));
            return PlayState.CONTINUE;
        }
        if (event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.horseshoe_crab.walk", true));
            return PlayState.CONTINUE;
        }
        event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.horseshoe_crab.idle", true));
        return PlayState.CONTINUE;
    }
